...
import { Urgency } from './models/task.interface';
export class InMemoryDataService implements InMemoryDbService {
    createDb() {
        const tasks = [
            {id: 100, name: 'Купить продуктов', description: 'Небыл уже неделю, давно sf', status: true, urgency: Urgency.Hi, finishTo: new Date('1/1/16') },
            {id: 101, name: 'Buy product', description: 'Небыл уже неделю, давно полра', status: false, urgency: Urgency.Low, finishTo: new Date('5/2/16')},
            ...
            {id: 107, name: 'Test8', description: '', status: true, urgency: Urgency.Hi, finishTo: new Date('1/12/15')},
        ];
        return {tasks};
    }
}